...
int* bcst_create(const char* pattern, int pattern_len) {
    // TODO BCST creation
    int* bcst=calloc(26,sizeof(int));
    for(int i=0;i<26;i++){
        bcst[i]=pattern_len;
    }
    for(int j=0;j<pattern_len-1;j++){
        bcst[pattern[j]-'a']=pattern_len-1-j;
    }
    return bcst;
}
...
int boyer_moore(const char* text, const char* pattern, int* bcst, int* gsst, FILE* output_file) 
{
    ...
    while (pos < text_len) {
        ...
        match=0;
        while(match<pattern_len && pattern[pattern_len-1-match]==text[pos-match]){
            match++;
        }
        ...
        d1=fmax(bcst[text[pos]-'a']-match,1);
        d2=(match>0)?gsst[match]:0;
        pos+=fmax(d1,d2);
    }
    fprintf(output_file,"\n");
    return comparisons;
...
int main() {
    ...
    FILE *output_file = fopen("boyermoore_output_PES2UG23CS272.txt", "w");
    FILE *values_file = fopen("boyermoore_values_PES2UG23CS272.txt", "w");
